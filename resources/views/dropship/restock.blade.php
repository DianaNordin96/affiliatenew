@inject('category', 'App\Http\Controllers\Dropship\ManageStockController')
    ...
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            Sort By: <select onchange="location = this.value;" class="form-control">
                    <option value="/product-dropship" selected>All Products</option>
                    @foreach ($prodCat as $item)
                    <option value="/product-dropship/{{$item->category}}/{{ $item->id }}" @if ($catID == $item->id){ selected } @endif>{{$item->category}}
                    </option>
                    @endforeach
                </select>
            <br/><br/>
                            ...
                            <div class="card-body">
                                <div class="new-arrival-product">
                                    ...
                                    <div class="new-arrival-content text-center mt-3">
                                        ...
                                        <span class="price">RM
                                            {{ number_format($product->price_hq + $product->price_shogun+ $product->price_damio + $product->price_merchant, 2) }}</span>
                                    </div>
                                    <div class="row">
                                        <div style="margin: auto">
                                            <button type="button"
                                                class="btn btn-success" title="Add to cart"
                                                onclick="window.location='{{ url('addToCartDropship/' . $product->id) }}'">
                                            ...
                                            &nbsp;<button style="color:white;" type="button" title="View"
                                                data-toggle="modal" onclick="openModalView(
                                                                ...
                                                                '{{ number_format($product->price_hq + $product->price_shogun + $product->price_damio + $product->price_merchant,  2) }}',
                                                                '{{ number_format($product->product_price - $product->price_hq - $product->price_shogun - $product->price_damio - $product->price_merchant, 2) }}',
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ url('addToPaymentCart-dropship') }}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="row">
    ...
    <script type="text/javascript">
        ...
        $(".remove-from-cart").click(function(e) {
            ...
            if (confirm("Are you sure")) {
                $.ajax({
                    url: '{{ url('remove-from-cartDropship') }}',
                    method: "DELETE",
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: ele.attr("data-id")
